...
写作 ──► /admin/（后台，浏览器里直接调 GitHub API）
   ...
   Cloudflare 构建（npm run build）
          │
          ▼
   Workers 静态资源层 ──────────────► 文章、CSS、图片、Pagefind 索引……
          └─ /img/* 仍走 worker/index.js ──► R2 桶（只为老地址兜底）
```

全链路没有数据库，也没有需要盯着的服务器进程。

图片 2026-08-26 从 R2 搬回了仓库（`src/content/posts/img/`），由 Astro 在
构建时处理 —— 它自己量尺寸、自己出各档缩略图、自己加内容哈希。
换来的是写文章时不用再管文件名、尺寸和上传，代价是图片进 git（当时 19 张
共 1.2 MB）。

[`worker/index.js`](worker/index.js) 和 R2 桶都**留着没动**，
`/img/...` 那批老地址照常能开 —— 站外可能有人引用过。新图不再往那儿传。
...
| [图片工作流](docs/图片工作流.md) | 图片放哪、怎么在文章里引用、封面图要求 |
...
- **图片（`src/content/posts/img/`）不在 CC 授权范围内。**
  2026-08-26 起图片存在仓库里，但许可上被明确排除 —— 里面混着公有领域的
  航天照片、按合理使用收录的书影和动画截图、以及本站自制的图表，
  权利状态完全不同，逐张单说。
